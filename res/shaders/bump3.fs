...
uniform sampler2D giTex1, giTex2, giTex3, giTex4;
...
/*
	if(!gIed){
		...
		spec=dif*pow(max(0., 
			dot(-reflect(-normalize(eyePos),bNormal),
			normalize( gl_LightSource[0].position.xyz))), specPower);
		dif.xyz+=gis.xyz*clamp(nMap.z, 0., 1.);
		
		gis=texture2D(giTex1, texCoord.zw); gis*=2.;
		dif.xyz+=gis.xyz*clamp(nMap.x, 0., 1.);

		gis=texture2D(giTex2, texCoord.zw); gis*=2.;
		dif.xyz+=gis.xyz*clamp(nMap.y, 0., 1.);

		gis=texture2D(giTex3, texCoord.zw); gis*=2.;
		dif.xyz+=gis.xyz*clamp(-nMap.x, 0., 1.);

		gis=texture2D(giTex4, texCoord.zw); gis*=2.;
		dif.xyz+=gis.xyz*clamp(-nMap.y, 0., 1.);
	//}
